...
public class App {
    ...
    private static void saveSandwich(AddSandwich addSandwichs) {
        File file = new File(CSV_FILE2);
        boolean fileExists = file.exists();
        try (PrintWriter writer = new PrintWriter(new FileWriter(CSV_FILE2, true))) {
            if (!fileExists) {
    ...
    private static void new_order(Scanner scanner) {
        ...
        while (inNewOrderMenu) {
            ...
            switch (option) {
                case "1":
                    Double a = new Sandwich().Sandwich();

                    AddSandwich order = new AddSandwich(LocalDateTime.now(), "Sandwich",a );
                    saveSandwich(order);
                    System.out.println("Sandwich saved successfully to order.");
                    break;
                case "2":
                    ...
                    break;
                case "3":
                    AddChips order2 = new AddChips(LocalDateTime.now(), "chips", 1.50);
                    saveChips(order2);
